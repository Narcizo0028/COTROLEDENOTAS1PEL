...
def is_defesa_pessoal(subject):
    ...
    name = unicodedata.normalize("NFKD", str(subject["name"] if subject else "")).encode("ascii", "ignore").decode().casefold()
        ...
        try:
            with connect() as db:
                if self.path=="/api/admin/student-entry":
                    ...
                    elif sub[1]==1: exam1=None;exam2=number("exam2",7);work=number("work",3)
